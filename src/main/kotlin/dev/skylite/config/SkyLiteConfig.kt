...
class SkyLiteConfig {
    ...
    companion object {
        ...
        private val handler: ConfigClassHandler<SkyLiteConfig> =
            ConfigClassHandler.createBuilder(SkyLiteConfig::class.java)
                .id(Identifier.fromNamespaceAndPath(SkyLite.MOD_ID, "config"))
                .serializer { definition ->
                    GsonConfigSerializerBuilder.create(definition)
                        .setPath(
                            FabricLoader.getInstance()
                                .configDir
                                .resolve("${SkyLite.MOD_ID}/config.json")
                        )
                        .setJson5(false)
                        .build()
                }
        ...
        val instance: SkyLiteConfig get() = handler.instance()

        fun handler(): ConfigClassHandler<SkyLiteConfig> = handler

        fun load() {
            handler.load()
            SkyLite.logger.debug("config loaded, enabled={}", instance.enabled)
        }
