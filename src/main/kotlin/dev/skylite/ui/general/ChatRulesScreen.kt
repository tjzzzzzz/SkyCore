...
class ChatRulesScreen(private val parent: Screen?) : Screen(Component.literal("Chat Rules")) {
    ...
    private var editing: SkyLiteConfig.ChatRuleEntry? = null
    ...
    private fun drawEditor(g: GuiGraphicsExtractor, rule: SkyLiteConfig.ChatRuleEntry, mouseX: Int, mouseY: Int, dt: Float) {
    ...
    private fun listClick(event: MouseButtonEvent): Boolean {
        ...
        if (Ui.inBounds(event.x, event.y, panelX + panelW - PAD - addW, panelY + 12, addW, BTN_H)) {
            val rule = SkyLiteConfig.ChatRuleEntry()
            rules.add(rule)
            ChatRules.save()
            open(rule)
            return true
    ...
    private fun editorClick(event: MouseButtonEvent, rule: SkyLiteConfig.ChatRuleEntry): Boolean {
        focusedField = -1
        var y = panelY + 42
        val w = panelW - PAD * 2
    ...
    private fun open(rule: SkyLiteConfig.ChatRuleEntry) {
        editing = rule
        nameBuf = rule.name
        matchBuf = rule.match
        titleBuf = rule.title
    ...
    private fun commit(rule: SkyLiteConfig.ChatRuleEntry) {
        rule.name = nameBuf
        rule.match = matchBuf
        rule.title = titleBuf
        rule.sound = soundBuf
