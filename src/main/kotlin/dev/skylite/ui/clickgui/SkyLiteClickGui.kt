...
import dev.skylite.ui.hud.HudEditorScreen
import dev.skylite.ui.render.Icons
...
/**
 * the skylite click gui.
 *
 * built to read as a desktop overlay rather than a game menu: a floating card
 * with an ambient shadow, a proper title bar, an icon rail down the left and
 * tactile toggle cards on the right. every glyph is plus jakarta sans through
 * the vanilla ttf pipeline, so nothing is a scaled bitmap.
 *
 * the whole screen is fills plus cached text components. no textures, no extra
 * draw calls, no per frame allocation.
 */
class SkyLiteClickGui : Screen(Component.literal("SkyLite")) {
    ...
    private companion object {
        ...
        /** logo lives at assets/skylite/art, 32x32 rgba drawn at half size */
        val ICON: Identifier = Identifier.fromNamespaceAndPath(SkyLite.MOD_ID, "art/gui-icon.png")
        const val ICON_SRC = 32
        const val ICON_DRAW = 16

        // cached chrome, built once per class load
        val TITLE: Component = Fonts.label("SkyLite", Fonts.SEMIBOLD)
        val VERSION: Component = Fonts.label("v${SkyLite.version}", Fonts.SMALL)
    ...
    /** keybind badge labels change rarely, rebuild only when the bind changes */
    ...
    /** results are recomputed when the query changes, never per frame */
    ...
    /**
     * expansion state while searching is kept separate from [GuiOption.expanded]
     * so opening a module to see the sub option you searched for does not
     * silently rearrange the normal settings list once the query is cleared.
     */
    ...
    private fun refreshSearch() {
        ...
        // if the query only matched through a sub option, open the module so the
        // thing you actually searched for is visible instead of hidden
    ...
    /** walks search rows, parents followed by children when opened */
    ...
    // a click gui on a live server must never pause or the player just stands there
    ...
    override fun extractRenderState(g: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, partialTick: Float) {
        ...
        // depth first: shadow, then a hairline accent bloom, then the card
        ...
        // the 1px outer border goes last so nothing paints over it
        Ui.frame(g, windowX, windowY, WINDOW_W, WINDOW_H, Theme.BORDER, RADIUS)
    }

    /** the rail sits a shade above the card so the split reads without a divider */
    private fun drawSidebarPlate(g: GuiGraphicsExtractor) {
        ...
        // square off the inner edge, only the outer corners should be round
    ...
    private fun drawTitleBar(g: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, dt: Float) {
        ...
        // 32px source drawn at 16px, an exact half so the downsample stays crisp
        g.blit(
        ...
        // search affordance, right aligned next to the close button
        val closeX = windowX + WINDOW_W - PAD - 11
        // fixed width so typing does not make the box grow into the title
        ...
        // close button, tints red on hover like a real window control
    ...
    private fun drawSidebar(g: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, dt: Float) {
        ...
        for ((index, category) in GuiCategory.entries.withIndex()) {
            ...
            // the glowing 2px rail is what marks the active tab
            if (active || fade > 0.6f) {
    ...
    /** lives at the foot of the rail so it reads as a tool, not a category */
    ...
    private fun drawOptions(g: GuiGraphicsExtractor, mouseX: Int, mouseY: Int, dt: Float) {
        ...
        forEachVisibleRow(options) { option, depth, offsetY ->
            ...
            if (depth == 0) {
                ...
                // children are one line, the parent already carries the context
                g.fill(x - 10, rowY + 4, x - 9, rowY + rowH - 4, Theme.BORDER_SOFT)
                g.text(font, option.titleLabel, x + 12, rowY + (rowH - 8) / 2, titleColor, false)
            }

            when (option) {
                // right edges of the toggle and the keybind badge line up at -12
                is ToggleOption -> {
    ...
    /**
     * flat result list. every hit shows which category it came from, since the
     * results deliberately cut across all of them.
     */
    private fun drawSearchResults(
        ...
        forEachSearchRow { option, hit, depth, offsetY ->
            ...
            if (depth == 0) {
                ...
                // breadcrumb so a hit from another tab is not disorienting
                if (hit != null) {
    ...
    /**
     * walks the rows that are currently on screen, parents followed by their
     * children when expanded. inline so the callback never allocates.
     */
    ...
    /**
     * checkbox rather than a switch. at 18px a sliding indicator can only ever
     * look like a toy, whereas a square that fills with a tick is exactly what
     * linear and raycast do and it stays legible.
     */
    ...
    /** shortcut badge, styled like a desktop key cap */
    ...
    /** opens a live editor screen, same affordance language as a keybind badge */
    ...
    /**
     * compact track + value. lives on the right of a child row so the title
     * still has room to breathe on the left.
     */
    ...
    /** badge text only changes on a rebind, so it is cached against the bind */
    ...
    override fun mouseClicked(event: MouseButtonEvent, doubleClick: Boolean): Boolean {
        ...
        // right click anywhere on a parent row expands or collapses it
        ...
        if (event.button() == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
            ...
            if (Ui.inBounds(mx, my, paneX, paneY, paneW, paneH)) {
                ...
                forEachVisibleRow(ClickGuiRegistry.optionsFor(selected)) { option, depth, offsetY ->
                    ...
                    // the chevron area expands, the rest of the row still toggles
                    if (depth == 0 && option.hasChildren && mx < paneX + PAD + 22) {
                        option.expanded = !option.expanded
                        return@forEachVisibleRow
                    }
    ...
    override fun keyPressed(event: KeyEvent): Boolean {
            ...
            when (event.key()) {
                GLFW.GLFW_KEY_BACKSPACE -> {
                    if (searchQuery.isNotEmpty()) {
                        // drop a whole codepoint, not half a surrogate pair
                ...
                // first escape clears the query, a second one closes the screen
                ...
                GLFW.GLFW_KEY_ENTER, GLFW.GLFW_KEY_KP_ENTER -> {
                    // enter toggles the top hit, which makes the search a launcher
        ...
        if (pending != null) {
            // swallow everything while listening, otherwise binding escape or
            // right shift would close the screen instead of binding
            listening = null
            when (event.key()) {
                GLFW.GLFW_KEY_ESCAPE -> Unit
                GLFW.GLFW_KEY_DELETE -> SkyLiteKeys.rebind(pending.mapping, InputConstants.UNKNOWN)
                else -> SkyLiteKeys.rebind(pending.mapping, InputConstants.getKey(event))
            }
            return true
        }

        // pressing the bound open key closes, same as esc. unbound keys stay inert.
        if (!searchFocused) {
            val bound = KeyMappingHelper.getBoundKeyOf(SkyLiteKeys.openClickGui)
