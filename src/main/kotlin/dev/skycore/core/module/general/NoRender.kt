...
object NoRender {
    ...
    fun shouldCancelEntity(entity: Entity): Boolean {
        ...
        val cfg = SkyCoreConfig.instance.noRender
    ...
    fun shouldHideFireOverlay(): Boolean = enabled() && SkyCoreConfig.instance.noRender.fireOverlay

    fun shouldHideBossBar(): Boolean = enabled() && SkyCoreConfig.instance.noRender.bossBar

    fun shouldHideFog(): Boolean = enabled() && SkyCoreConfig.instance.noRender.fog

    fun shouldHideNausea(): Boolean = enabled() && SkyCoreConfig.instance.noRender.nausea

    fun shouldHideBreakParticles(): Boolean = enabled() && SkyCoreConfig.instance.noRender.breakParticles

    fun shouldHideArmorBar(): Boolean = enabled() && SkyCoreConfig.instance.noRender.armorBar

    fun shouldHideFoodBar(): Boolean = enabled() && SkyCoreConfig.instance.noRender.foodBar

    fun shouldHideEffectDisplay(): Boolean = enabled() && SkyCoreConfig.instance.noRender.effectDisplay

    fun shouldHideSelectedItemName(): Boolean = enabled() && SkyCoreConfig.instance.noRender.selectedItemName

    fun shouldHideRecipeBook(): Boolean = enabled() && SkyCoreConfig.instance.noRender.recipeBook

    fun shouldHideEmptyTooltips(slot: Slot?, title: String): Boolean {
        if (title.startsWith("Ultrasequencer (")) return false
        return enabled() &&
            SkyCoreConfig.instance.noRender.emptyTooltips &&
            slot != null &&
            slot.item.hoverName.string.trim().isEmpty()
    }

    fun shouldHideEntityFire(): Boolean = enabled() && SkyCoreConfig.instance.noRender.entityFire

    fun shouldHideSoulweaverSkulls(): Boolean = enabled() && SkyCoreConfig.instance.noRender.soulweaverSkulls

    fun shouldHideHealerFairy(): Boolean = enabled() && SkyCoreConfig.instance.noRender.healerFairy
    ...
    private fun enabled(): Boolean =
        SkyCoreConfig.instance.enabled && SkyCoreConfig.instance.noRender.enabled
