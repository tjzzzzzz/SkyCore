...
object SlotBinding {
    ...
    private val opts get() = SkyCoreConfig.instance.slotBinding

    fun init() {
        slotBindingKey = KeyMappingHelper.registerKeyMapping(
            KeyMapping(
                "key.skycore.slot_binding",
    ...
    private fun handleRelease(focusedId: Int) {
        ...
        val data = opts.binds.getOrPut(name) { SkyCoreConfig.HotbarBind() }
    ...
    private fun hotbarBind(slotId: Int): SkyCoreConfig.HotbarBind =
        opts.binds.getOrPut("hotbar${toHotbar(slotId)}") { SkyCoreConfig.HotbarBind() }
